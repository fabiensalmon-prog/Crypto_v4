# HELIOS ONE — V4.3 PRO (single file, full features)
# - Sélection par trade (checkbox + bouton "Prendre ce trade")
# - Lab rapide par ligne (🔬)
# - Équity bien visible, cap d’exposition global + cap par trade (avec levier par mode)
# - Modes: Conservateur / Normal / Agressif / Super agressif (levier x5)
# - Ensemble 20+ strats, gating HTF + macro (VIX/Gold)
# - Sizing au risque %, Multi-TP (TP1->BE, TP2->trail, TP3->full)
# - Auto-close TP/SL, normalisation META, fix SL côté entry, historique & métriques
# - Manual only (aucune exécution automatique d’ordres réels)
...
try:
    ...
    pass
st.set_page_config(page_title="HELIOS ONE — V4.3 PRO", page_icon=ICON, layout="centered")
st.title("HELIOS ONE — V4.3 PRO")
...
# ───────────── Exchange helpers (fallback multi-venue) ─────────
...
def load_or_fetch(exchange_id: str, symbol: str, tf: str, limit=1500) -> pd.DataFrame:
    ...
    for ex in [exchange_id] + [e for e in FALLBACK if e != exchange_id]:
        try:
            return fetch_ohlcv(ex, symbol, tf, limit)
...
def sig_ichimoku(df,conv=9,base=26,spanb=52):
    ...
    spanA=((ten+kij)/2).shift(base)
    high52=df['high'].rolling(spanb).max(); low52=df['low'].rolling(spanb).min(); spanB=((high52+low52)/2).shift(base)
    cross=(ten>kij).astype(int)-(ten<kij).astype(int)
    up=(df['close']>spanA)&(df['close']>spanB); dn=(df['close']<spanA)&(df['close']<spanB)
    sig=cross.where(up,0).where(~dn,-1)
...
def sig_psar(df,af=0.02,max_af=0.2):
    h,l=df['high'],df['low']; ps=l.copy()
    bull=True; a=af; ep=h.iloc[0]; ps.iloc[0]=l.iloc[0]
...
def ensemble_weights(df, signals, window=300):
    ...
    for n,s in signals.items():
        try:
            _,_,p,eq = compute(df.iloc[start:], s.iloc[start:])
            sc[n] = _score(p,eq)
        except Exception:
            sc[n] = -1e9
    keys = list(sc.keys())
    arr = np.array([sc[k] for k in keys])
...
def htf_gate(df_ltf, df_htf):
    return sig_ema_trend(df_htf).reindex(df_ltf.index).ffill().fillna(0.0)
...
def macro_gate(enable, vix_caution=20.0, vix_riskoff=28.0, gold_mom_thr=0.10):
    ...
    if lvl > float(vix_riskoff):
        mult *= 0.0; note.append(f"VIX>{vix_riskoff} risk-off")
    elif lvl > float(vix_caution):
        mult *= 0.5; note.append(f"VIX>{vix_caution} caution")
    else:
        note.append("VIX benign")
    if gold is not None and not gold.empty:
        mom = float(gold.pct_change(63).iloc[-1])
        if mom > float(gold_mom_thr):
            mult *= 0.8; note.append("Gold strong")
    return mult, " | ".join(note)

# ───────────── Risk, sizing & R-multiples levels ──────────────
def atr_levels(df, d, sl_mult=2.5, tp_mult=4.0):
    if d==0 or len(df)<2: return None
    a = float(atr_df(df,14).iloc[-1])
...
def list_positions(status=None, limit=100000) -> pd.DataFrame:
    _init_db(); conn=sqlite3.connect(DB)
...
def close_position(pid, px, note='CLOSE'):
    ...
    row=c.execute('SELECT open_ts,symbol,side,entry,sl,tp,qty FROM positions WHERE id=? AND status="OPEN"', (pid,)).fetchone()
    if not row: conn.close(); return None
    _,symbol,side,entry,_,_,qty = row
    ...
    c.execute('UPDATE positions SET close_ts=?, status=?, exit_price=?, pnl=?, note=? WHERE id=?',
              (datetime.datetime.utcnow().isoformat(),"CLOSED",float(px),float(pnl),note,pid))
    conn.commit(); conn.close()
    return pnl
...
def partial_close(pid, px, qty_close, reason="TP"):
    ...
    c.execute('INSERT INTO positions (open_ts,close_ts,symbol,side,entry,sl,tp,qty,status,exit_price,pnl,note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
              (open_ts,datetime.datetime.utcnow().isoformat(),symbol,side,float(entry),float(sl),float(tp),float(qty_close),"CLOSED",float(px),float(pnl),reason))
    remain = float(qty)-qty_close
    if remain>1e-12:
        c.execute('UPDATE positions SET qty=? WHERE id=? AND status="OPEN"', (remain, pid))
    else:
        c.execute('UPDATE positions SET close_ts=?, status=?, exit_price=?, pnl=?, note=? WHERE id=?',
                  (datetime.datetime.utcnow().isoformat(),"CLOSED",float(px),float(pnl),reason+"(FULL)",pid))
    conn.commit(); conn.close()
    return pnl

# ───────────── Multi-TP helpers (R multiples) ────────────────
...
def build_meta_r(entry, sl, side, qty, splits=(0.4,0.4,0.2), tpR=(1.0,2.0,3.5), be_after_tp1=True):
    ...
    q3 = max(0.0, q3+diff)  # somme = qty
    return {
        'multi_tp': True, 'mode':'R', 'tpR': list(tpR), 'splits': list(splits),
        'targets': [
            {'name':'TP1','px':tps[0],'qty':q1,'filled':False},
...
def normalize_meta(meta, qty, entry, sl, side, default_splits=(0.4,0.4,0.2), default_tpR=(1.0,2.0,3.5)):
    try:
        if not isinstance(meta, dict) or not meta.get('multi_tp'):
            return build_meta_r(entry, sl, side, qty, splits=default_splits, tpR=default_tpR)
        splits = tuple(meta.get('splits', default_splits))
        tpR    = tuple(meta.get('tpR', default_tpR))
        tgt = meta.get('targets', [])
        if (not isinstance(tgt, list)) or (len(tgt)==0) or isinstance(tgt[0], (int,float,str)):
            return build_meta_r(entry, sl, side, qty, splits=splits, tpR=tpR, be_after_tp1=bool(meta.get('be_after_tp1', True)))
        want = r_targets(entry, sl, side, tpR)
        for i in range(min(3,len(tgt))):
            if not isinstance(tgt[i], dict): tgt[i] = {}
            tgt[i].setdefault('name', f"TP{i+1}")
            tgt[i].setdefault('px',   want[i])
            tgt[i].setdefault('qty',  max(0.0, qty*(splits[i] if i < len(splits) else 0.0)))
            tgt[i].setdefault('filled', False)
        meta['targets'] = tgt[:3]
        meta.setdefault('be_after_tp1', True)
        meta.setdefault('trail_after_tp2', True)
        meta['tpR'] = list(tpR); meta['splits'] = list(splits)
        return meta
    except Exception:
        return build_meta_r(entry, sl, side, qty, splits=default_splits, tpR=default_tpR)

# ───────────── Auto-manage (TP/SL/BE/Trail + sanitize) ───────
...
def _chandelier_stop(df, n=22, k=3.0, side="LONG"):
    ...
    if side.upper()=="LONG":
        return (df["high"].rolling(n).max().values - k*atr)
    else:
        return (df["low"].rolling(n).min().values + k*atr)
...
def sanitize_all_positions():
    ...
    for _,r in df[df["status"]=="OPEN"].iterrows():
        ...
        if side=="LONG" and sl>=entry:
            new = entry - max(1e-9, abs(sl-entry)); fix=True
        if side=="SHORT" and sl<=entry:
            new = entry + max(1e-9, abs(sl-entry)); fix=True
        if fix:
            conn=sqlite3.connect(DB)
            conn.execute('UPDATE positions SET sl=? WHERE id=?', (float(new), int(r['id'])))
            conn.commit(); conn.close(); changed=True
    return changed

def auto_manage_positions(price_map, ohlc_map=None, mode="Normal", be_after_tp1=True, trail_after_tp2=True, fee_buffer_bps=0):
    sanitize_all_positions()
    df = list_positions(status='OPEN')
    if df.empty: return []
    # presets par mode (pour recalcul des TP R)
    if str(mode).lower().startswith("conserv"):
        parts=(0.50,0.35,0.15); tpsR=(0.9,1.7,2.6)
    elif str(mode).lower().startswith("aggr") and "super" not in str(mode).lower():
        parts=(0.30,0.40,0.30); tpsR=(1.0,2.5,5.0)
    elif "super" in str(mode).lower():
        parts=(0.34,0.33,0.33); tpsR=(1.2,3.0,6.0)
    else:
        parts=(0.40,0.40,0.20); tpsR=(1.0,2.0,3.5)
    evts=[]
    for _,r in df.iterrows():
        ...
        meta_raw=_meta_from_note(r['note'])
        meta=normalize_meta(meta_raw, qty, entry, sl, side)
        ...
        if not meta['targets'][0]['filled'] and hit_tp(px, meta['targets'][0]['px']):
            ...
            if q>0:
                partial_close(int(r['id']), px, q, "TP1")
                evts.append((sym, "TP1", px, q))
                meta['targets'][0]['filled']=True; meta['targets'][0]['qty']=0.0; changed=True
            if be_after_tp1:
                be_px = entry + (fee_buffer_bps/10000.0)*entry*(1 if side=="LONG" else -1)
                update_sl(int(r['id']), be_px)
        ...
        if not meta['targets'][2]['filled'] and hit_tp(px, meta['targets'][2]['px']):
            partial_close(int(r['id']), px, qty_left, "TP3")
            evts.append((sym,"TP3",px,qty_left))
        ...
        if not cur.empty:
            ...
            if qty_left>0 and hit_sl(px, stop):
                partial_close(int(r['id']), stop, qty_left, "SL")
                evts.append((sym,"SL",stop,qty_left))
                qty_left=0.0
...
def portfolio_equity(base_capital, price_map=None):
    ...
    closed_df = list_positions(status='CLOSED')
...
# valeurs par défaut si l’expander n’est pas ouvert
...
with tabs[0]:
    ...
    macro_mult, macro_note = macro_gate(macro_enabled)
    ...
    if st.button("🚀 Scanner maintenant", use_container_width=True):
        ...
        for sym in symbols:
            try:
                ...
                df_htf = load_or_fetch(exchange, sym, htf, 600)
            except Exception as e:
                st.warning(f"Skip {sym}: {e}"); continue
            # ensemble de strats (toutes)
            ...
            # seuil mode (moins de signaux mais plus sûrs quand Super)
        ...
        if not rows:
            ...
            st.markdown("#### Sélection (cocher/ajuster) + actions")
            edit = st.data_editor(
            ...
            for i, r in edit.iterrows():
                ...
                if c2.button("📌 Prendre ce trade", key=f"take_{i}"):
                    ...
                    if scale <= 0:
                        st.warning("Cap atteint."); st.rerun()
                    entry = float(r['entry'])
                    meta = build_meta_r(entry, float(r['sl']), r['dir'], qty*scale, splits=m['splits'], tpR=m['tpR'], be_after_tp1=True)
            ...
            if st.button("📌 Prendre la sélection"):
                if sel.empty: st.warning("Rien de sélectionné.")
                else:
                    total_alloc = float((sel['qty']*sel['entry']).sum())
                    scale = 1.0
                    ...
                    for _,r in sel.iterrows():
                        ...
                        qty = float(r['qty'])*scale
                        notional = qty*entry
                        if notional>per_trade_cap: qty *= per_trade_cap/max(notional,1e-9)
                        if qty<=0: continue
                        meta = build_meta_r(entry, float(r['sl']), r['dir'], qty, splits=m['splits'], tpR=m['tpR'], be_after_tp1=True)
    ...
    if open_df.empty:
        ...
        for _,r in open_df.iterrows():
            ...
            if isinstance(meta, dict) and meta.get('multi_tp'):
                meta = normalize_meta(meta, r['qty'], r['entry'], r['sl'], r['side']); _set_meta(int(r['id']), meta)
                for t in meta.get('targets', []):
                    if isinstance(t, dict):
                        tick = "✅" if t.get('filled') else "🟡"
                        nm = str(t.get('name','TP'))
                        px = float(t.get('px', r['tp']))
            ...
            if cols[2].button(f"Force {next_name}", key=f"force_{r['id']}", disabled=(next_qty<=0)):
                px=last.get(r['symbol'], r['entry'])
                partial_close(int(r['id']), float(px), float(next_qty), f"FORCE_{next_name}")
                if next_name=="TP1": update_sl(int(r['id']), float(r['entry']))
                st.rerun()
            ...
            if cols[3].button(f"Close {dyn}%", key=f"closep_{r['id']}", disabled=(next_qty<=0)):
                px=last.get(r['symbol'], r['entry'])
                partial_close(int(r['id']), float(px), float(r['qty'])*dyn/100.0, f"MANUAL_{dyn}")
                st.rerun()
            if cols[4].button("Trail", key=f"trail_{r['id']}"):
                df_now = load_or_fetch(exchange, r['symbol'], tf, 220)
                side = r['side'].upper()
                trail = float(_chandelier_stop(df_now,22,3.0,side)[-1])
                if (side=='LONG' and trail>r['sl']) or (side=='SHORT' and trail<r['sl']):
                    update_sl(int(r['id']), trail)
                st.rerun()
...
with tabs[2]:
    ...
    if hist.empty:
        ...
        st.dataframe(hist[['close_ts','exit_price','qty','symbol','pnl','note']], use_container_width=True)
