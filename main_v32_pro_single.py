...
try:
    ...
    st.error("ccxt manquant. Vérifie requirements.txt"); st.stop()
...
# yfinance pour VIX/Gold et daily BTC
try:
...
def cross_alphas(exchange: str, symbols: list):
    uni = {}
    for s in symbols:
        try:
            df = load_or_fetch(exchange, s, "1d", limit=400)[['close','volume']].rename(columns={'close':s+'_c','volume':s+'_v'})
            uni[s]=df
        except Exception:
            continue
    if not uni: return pd.DataFrame()
    df = pd.concat(uni.values(), axis=1).dropna(how='all')
    mom = {s: df[s+'_c'].pct_change(63).iloc[-1] if s+'_c' in df else np.nan for s in symbols}
    volz = {}
    for s in symbols:
        if s+'_v' in df:
            z = (df[s+'_v'] - df[s+'_v'].rolling(63).mean())/(df[s+'_v'].rolling(63).std()+1e-9)
            volz[s] = float(z.iloc[-1])
        else:
            volz[s] = np.nan
    out = pd.DataFrame({'mom63':mom, 'volz':volz})
    for col in out.columns:
        out[col] = (out[col] - out[col].min()) / (out[col].max()-out[col].min()+1e-9)
...
with tabs[0]:
    ...
    if st.button('🚀 Scanner maintenant'):
        ...
        if not rows: st.info("Aucun setup solide pour l’instant.")
        else:
            picks = (pd.DataFrame(rows).sort_values(['score','confiance','rr'], ascending=False)
                     .head(int(max_pos)))
            st.dataframe(picks[['symbol','dir','entry','sl','tp','qty','rr','confiance','xalpha','score']].round(6), use_container_width=True)
            ...
            st.dataframe(picks_alloc[['symbol','dir','entry','sl','tp','alloc_qty','rr','confiance']].round(6), use_container_width=True)
...
with tabs[1]:
    ...
    open_df = list_positions(status='OPEN', limit=500)
    if open_df.empty: st.info('Aucune position ouverte.')
    else:
        last_prices = {s: fetch_last_price(exchange, s) for s in open_df['symbol'].unique()}
        def latent(row):
            last = last_prices.get(row['symbol'], row['entry']); sign = 1 if row['side']=='LONG' else -1
            return (last-row['entry'])*row['qty']*sign
        open_df['last']=open_df['symbol'].map(last_prices); open_df['PnL_latent']=open_df.apply(latent, axis=1)
        st.dataframe(open_df[['id','symbol','side','entry','sl','tp','qty','last','PnL_latent']].round(6), use_container_width=True)
        ...
        for _, r in open_df.iterrows():
            ...
            with cols[0]: st.write(f"{r['symbol']} · {r['side']} — entry {r['entry']:.6f} qty {r['qty']:.4f}")
...
with tabs[2]:
    ...
    closed_df = list_positions(status='CLOSED', limit=100000)
    if closed_df.empty: st.info('Aucun trade clôturé.')
    else:
        st.dataframe(closed_df[['id','open_ts','close_ts','symbol','side','entry','exit_price','qty','pnl','note']], use_container_width=True)
